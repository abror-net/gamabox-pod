...
function run_test_case () {
  ...
  k8s_single_pod_ready -l app=zookeeper,release=sunday-morning
  k8s_all_pods_ready 3 -l app=hdfs-journalnode,release=sunday-morning
  k8s_all_pods_ready 2 -l app=hdfs-namenode,release=sunday-morning
  k8s_single_pod_ready -l app=hdfs-datanode,release=sunday-morning
  k8s_single_pod_ready -l app=hdfs-client,release=sunday-morning
  _CLIENT=$(kubectl get pods -l app=hdfs-client,release=sunday-morning -o name |  \
      cut -d/ -f 2)
...
function cleanup_test_case() {
  helm delete --purge sunday-morning || true
}
